...
import { IMessage, IMessageSendResponse, InboxResponse } from '../model/interface/message';
...
export class MessageService {
  ...
  getInbox(): Observable<IMessage[] | InboxResponse> {
    const token = this.getJwtToken();
    if (!token) {
    ...
    const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
    return this.http.get<IMessage[] | InboxResponse>(
